...
@Deprecated("Use [HrefOptionsDelegate]")
...
/**
 * This class allows you to set the href with an hRefOptions.
 * Note that it will actually write to the href and other props, not an hRefOptions prop.
 */
class HrefOptionsDelegate : ReadWriteProperty<Props, HRefOptions?> {
    override fun getValue(thisRef: Props, property: KProperty<*>): HRefOptions {
        error("Delegate can't read value of ${thisRef}.${property.name}")
    }

    override fun setValue(thisRef: Props, property: KProperty<*>, value: HRefOptions?) {
        if (property.name != "hrefOptions") error("Unexpected property name '${property.name}' in HrefOptionsDelegate (expecting 'hrefOptions')")

        if (value != null) {
            thisRef.asDynamic().href = value.href
            thisRef.asDynamic().component = "a"

            value.target?.let { thisRef.asDynamic().target = it }

            if (value.targetBlank) {
                thisRef.asDynamic().target = "_blank"
                thisRef.asDynamic().rel = "noopener"
            }
        } else {
            thisRef.asDynamic().href = undefined
        }
    }
}
...
class BreakpointNullToFalseDelegate : ReadWriteProperty<Props, Breakpoint?> {
    override fun getValue(thisRef: Props, property: KProperty<*>): Breakpoint? {
        if (thisRef.asDynamic()[property.name] == false) return null
        return Breakpoint.valueOf(thisRef.asDynamic()[property.name] as String)
    }
